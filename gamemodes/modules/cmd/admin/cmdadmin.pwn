...
Dialog:ADMIN_MENU(playerid, response, listitem, inputtext[])
{
	...
	if(response)
	{
		...
		switch (listitem)
		{
            ...
            case 6: //kick
            {
                return Dialog_Show(playerid, ADMIN_MENU_KICK, DIALOG_STYLE_INPUT, "Kick", "Name: %s\nกรุณาระบุสาเหตุการ 'Kick'","เตะ","ยกเลิก", GetRoleplayName(clickedplayerid));
            }
            case 7: //Ban
            {
                return Dialog_Show(playerid, ADMIN_MENU_BAN, DIALOG_STYLE_INPUT, "Ban", "Name: %s\nกรุณาระบุสาเหตุการ 'Ban'","แบน","ยกเลิก", GetRoleplayName(clickedplayerid));
            }
            case 8: //Check Stats
            {
            ...
            case 9: //Check Afk
            {
				if(GetTickCount() > (Character[clickedplayerid][PauseCheck]+2000))
					return SendClientMessageEx(playerid, COLOR_GREY, "ผู้เล่น %s ได้ AFK เป็นเวลา %i วินาที!", ReturnName(clickedplayerid), Character[clickedplayerid][PauseTime]);
            }
...
Dialog:ADMIN_MENU_BAN(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		new
		pID = playerid, 
		str[128], clickedplayerid = Character[playerid][AdminID];		
        ...
        format(str, sizeof(str), "[Administrator] : ได้แบนผู้เล่น %s ออกจากเซิฟเวอร์ | เนื่องจาก: %s", Account[clickedplayerid][Name], GetRoleplayName(clickedplayerid), inputtext);
        SendPunishmentMessage(str);
        IssueBan(pID, Account[clickedplayerid][Name], inputtext);
